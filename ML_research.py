...
def tf_nn_research(file, epochs=10, split=0.6, o=1):
    ...
    X = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, m])
    Y = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None])
    # initializers
    sigma = 1
    W = tf.compat.v1.variance_scaling_initializer(
    ...
    for e in range(epochs):
        ...
        for i in range(0, len(y_train) // batch_size):
            ...
            if np.mod(i, 50) == 0:
                ...
                mse_test.append(
                    sess.run(mse, feed_dict={X: X_test, Y: y_test}))
...
def plot_LinearRegression(file, o=5, split=0.6):
    ...
    print("Training sizes: X: %.2f, y: %.2f" %
...
def AdaBoost(file):
    ...
    model = ensemble.AdaBoostRegressor(n_estimators=5000, learning_rate=2.0, estimator=dtree)
    model.fit(X_train, y_train)
    score = model.score(X_test, y_test)
    ...
    print(df[['Open','High','Low','Volume']][n-1:])
