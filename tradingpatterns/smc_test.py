...
def add_OB(fig, df, ob_data):
    ...
    for i in range(len(ob_data["OB"])):
        if ob_data["OB"][i] == 1:
            ...
            # Add annotation text
            annotation_text = f'OB: {
                volume_text} ({ob_data["Percentage"][i]}%)'
    ...
    for i in range(len(retracements)):
        if (
            ...
            fig.add_annotation(
                ...
                xref="x",
                yref="y",
                text=f"C:{retracements['CurrentRetracement%'].iloc[i]}%<br>D:{
                    retracements['DeepestRetracement%'].iloc[i]}%",
                font=dict(color="rgba(255, 255, 255, 0.4)", size=8),
                showarrow=False,
            )
    return fig
...
def fig_to_buffer(fig: go.Figure):
